...
class ControllerExtensionModuleclassifiedlatest extends Controller {
	public function index($setting) {
		...
		$data['classifiedseachs']=array();
      	$category_searchs = $this->model_classified_classified_category->customerAllad($filter_data);
		foreach ($category_searchs as $category_search) {
		  ...
		  $totdate=date('Y-m-d');
			...
			$data['classifiedseachs'][]=array(
				...
				'post_description' => 
                utf8_substr(strip_tags(html_entity_decode($category_search['post_description'], ENT_QUOTES, 'UTF-8')), 0, 100) . '..', 
			...
			return $this->load->view('extension/module/classifiedlatest', $data);
